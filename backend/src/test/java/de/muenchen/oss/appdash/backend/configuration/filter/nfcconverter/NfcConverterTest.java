...
class NfcConverterTest {
  ...
  void givenContentTypeInWhitelist_thenFilter() throws ServletException, IOException {
    ...
    // Check
    final ArgumentCaptor<HttpServletRequest> reqCaptor =
    ...
    assertArrayEquals(
        ...
        StreamUtils.copyToByteArray(reqCaptor.getValue().getPart(NAME_NFD).getInputStream()));
  }

  // Test that Request not configured ContentType remains unchanged, i.e. is not
  // normalized to NFC.
  @Test
