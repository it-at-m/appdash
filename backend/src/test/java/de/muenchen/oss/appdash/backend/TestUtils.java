...
public final class TestUtils {
   ...
   *
   * <p><b>Note:</b> The tag is being removed due to Testcontainers not supporting supplying both
   * tag and SHA value.
   *
  ...
  public static String getImageFromDockerCompose(final String serviceName) {
    ...
    try (InputStream input = Files.newInputStream(Path.of("../stack/docker-compose.yml"))) {
      final Map<String, Object> compose = yaml.load(input);

      final Object image =
          ((Map<?, ?>) ((Map<?, ?>) compose.get("services")).get(serviceName)).get("image");

      if (!(image instanceof String)) {
        throw new IllegalStateException(
            String.format("Could not find services.%s.image in docker-compose.yml", serviceName));
      }

      return removeImageTag((String) image);
    } catch (IOException e) {
      throw new IllegalStateException("Could not read docker-compose.yml", e);
    }
  }
