 ...
 *
 * <p>The usage of default role-based authorization should be preferred.
 */
@Slf4j
@Component
@Profile("keycloak-permissions")
public final class KeycloakPermissionsAuthoritiesConverter
  ...
  public KeycloakPermissionsAuthoritiesConverter(
    ...
    Assert.hasText(
        securityProperties.getPermissionsUri(),
        "permissionsUri must be set to use keycloak-permissions profile");
    this.securityProperties = securityProperties;
    this.restClient =
        restClientBuilder.requestFactory(requestFactory(requestFactoryBuilder)).build();
    this.cache =
        new CaffeineCache(
  ...
  /* package */ KeycloakPermissionsAuthoritiesConverter(
      final SecurityProperties securityProperties, final RestClient restClient, final Cache cache) {
    this.securityProperties = securityProperties;
  ...
  public Collection<GrantedAuthority> convert(final Jwt jwt) {
    ...
    if (valueWrapper != null) {
      ...
      final Collection<GrantedAuthority> authorities =
          (Collection<GrantedAuthority>) valueWrapper.get();
    ...
    try {
      final List<Map<String, Object>> permissions =
          this.fetchPermissions(
              this.securityProperties.getPermissionsUri(),
              jwt,
  ...
  private List<Map<String, Object>> fetchPermissions(
      final String endpointUrl, final Jwt jwt, final String clientId) {
    try {
