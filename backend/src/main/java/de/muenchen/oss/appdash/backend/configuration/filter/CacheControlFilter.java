...
/**
 * The filter adds a {@link HttpHeaders#CACHE_CONTROL} header to each HTTP response, if the header
 * is not already set.
 */
@Component
...
public class CacheControlFilter extends OncePerRequestFilter {
   ...
   * HttpServletResponse} given in the parameter, if the header is not already set. Same contract as
   * for {@code super.doFilter}, but guaranteed to be just invoked once per request within a single
   * request thread. See {@link OncePerRequestFilter#shouldNotFilterAsyncDispatch()} for details.
   *
