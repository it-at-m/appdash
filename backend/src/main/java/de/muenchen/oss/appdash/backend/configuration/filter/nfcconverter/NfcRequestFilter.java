package de.muenchen.oss.appdash.backend.configuration.filter.nfcconverter;
 ...
 *       The reason for this is that binary data streams are transferred here and these are
 *       generally not simple text. If necessary or useful, the application logic or a suitable
 *       library can or must carry out NFC normalization.
 *   <li>NFC normalization can only be performed at the character level and the conversion of binary
...
public class NfcRequestFilter extends OncePerRequestFilter {
  ...
  private static final Set<String> CONTENT_TYPES =
      new HashSet<>(Arrays.asList("text/plain", "application/json", "text/html"));
  ...
  protected void doFilterInternal(
    ...
    if (contentType != null && CONTENT_TYPES.stream().anyMatch(contentType::startsWith)) {
      ...
      log.debug(
          "Skip processing of HTTP request since it's content type \"{}\" is not in"
              + " whitelist.",
          contentType);
