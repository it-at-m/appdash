...
import de.muenchen.oss.appdash.backend.security.AuthUtils;
...
public class RequestResponseLoggingFilter extends OncePerRequestFilter {
   ...
   * HTTP-Request, the targeted URI and the response http status code. {@inheritDoc}
   */
  @Override
  ...
  private boolean checkForLogging(final HttpServletRequest httpServletRequest) {
    final boolean isLoggingMode =
        switch (securityProperties.getLoggingMode()) {
          case ALL -> true;
          case CHANGING -> CHANGING_METHODS.contains(httpServletRequest.getMethod());
          default -> false;
        };

    return isLoggingMode
        && securityProperties.getLoggingIgnoreListAsMatchers().stream()
            .noneMatch(matcher -> matcher.matches(httpServletRequest));
  }
}
