...
import de.muenchen.oss.appdash.backend.configuration.filter.RequestResponseLoggingFilter;
...
public class SecurityProperties {
  ...
  /**
   * Name of the JWT claim that contains the username. Defaults to "preferred_username" if not
   * configured.
   */
  @NotBlank private String usernameClaim = "preferred_username";

  /**
   * URI of the endpoint used for fetching permissions, see also {@link
   * KeycloakPermissionsAuthoritiesConverter}.
   */
  private String permissionsUri;

  /**
   * Timeout for which resolved permissions are cached and reused. Default 60s. See {@link
   * KeycloakPermissionsAuthoritiesConverter}
   */
  ...
  @NotNull private RequestResponseLoggingFilter.LoggingMode loggingMode =
      RequestResponseLoggingFilter.LoggingMode.NONE;

  /**
   * List of paths to ignore when logging HTTP requests, see also {@link
   * RequestResponseLoggingFilter}
   */
  @NotNull private List<PathPatternRequestMatcher> loggingIgnoreList =
      List.of(PathPatternRequestMatcher.withDefaults().matcher("/actuator/**"));
  ...
  public List<String> getLoggingIgnoreList() {
    return loggingIgnoreList.stream()
        .map(PathPatternRequestMatcher::toString)
        .collect(Collectors.toList());
  }

  public void setLoggingIgnoreList(final List<String> patterns) {
    this.loggingIgnoreList =
        patterns.stream()
